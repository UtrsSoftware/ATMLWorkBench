...
AtlasExecute& AtlasExecute::operator = ( const AtlasStatement& other )
{
	if ( this != &other )
	{
		...
		const AtlasExecute* pother = dynamic_cast< const AtlasExecute* >( &other );
...
void AtlasExecute::InitFromXML( const StatementMetadata* pData )
{
	if ( 0 != pData )
	{
...
void AtlasExecute::Process( const Lookup* pLookup )
{
...
void AtlasExecute::ToXML( string& strXML ) const
{
	const string strId( XML::MakeXmlAttributeNameValue( XML::anId, m_uiId ) );

	strXML += XML::MakeOpenXmlElementWithChildren( XML::enExecute, strId, GetStatementCommentRefId_XML( ) );
	...
	strXML += XML::MakeCloseXmlElementWithChildren( XML::enExecute );
}