...
AtlasGoto& AtlasGoto::operator = ( const AtlasStatement& other )
{
	if ( this != &other )
	{
		...
		const AtlasGoto* pother = dynamic_cast< const AtlasGoto* >( &other );

		if ( 0 != pother )
		{
			m_strStatementNumber = pother->m_strStatementNumber;
			m_uiRefId = pother->m_uiRefId;
			m_uiProcRefId = pother->m_uiProcRefId;
		}
	}
...
void AtlasGoto::InitFromXML( const StatementMetadata* pData )
{
	if ( 0 != pData )
	{
...
void AtlasGoto::Process( const Lookup* pLookup )
{
...
void AtlasGoto::ToXML( string& strXML ) const
{
	...
	if ( m_uiProcRefId != m_uiParentProcedureId )
	{
		strProcRefId = XML::MakeXmlAttributeNameValue( XML::anProcRefId, m_uiProcRefId );
	}
	...
	strXML += XML::MakeXmlElementNoChildren( XML::enTarget, XML::MakeXmlAttributeNameValue( XML::anStatementNumber, m_strStatementNumber ), strProcRefId, strRefId );
